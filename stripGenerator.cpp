...
// turns the given float numbers into string
...
// turns the given vertex array into a new array with normals generated for each vertex, and prints it out
void generateStripNormals(float* arr, int totalFloats) {
    int vertexCount = totalFloats / 8;
    map<string, vector<glm::vec3>> accumulated;

    for (int i = 0; i + 2 < vertexCount; i++) {
        if (i % 2 == 0)
            accumulateTriangle(arr, i,   i+1, i+2, accumulated);
        else
            accumulateTriangle(arr, i+1, i,   i+2, accumulated);
    }
...
int main() {
    generateStripNormals(vectorArray, sizeof(vectorArray) / sizeof(float));
    return 0;
}